...
#define SPINNER_HEADER(pos, size, centre, num_segments) ImVec2 pos, size, centre; int num_segments; if (!detail::SpinnerBegin(label, radius, pos, size, centre, num_segments)) { return; }; ImGuiWindow *window = ImGui::GetCurrentWindow();
    ...
    {
        {
            ...
            const float nextItemKoeff = 2.7f;
            const float heightKoeff = 2.f;
            const float heightSpeed = 0.8f;
            ImVec2 pos = window->DC.CursorPos;
            ImVec2 size((thickness * nextItemKoeff) * 3 + style.FramePadding.x,
                thickness * 4 * heightKoeff + style.FramePadding.y);
            ...
            // Render
            float start = (float)ImGui::GetTime() * 5.8f;

            const float offset = IM_PI / 3;
            for (size_t i = 0; i < 3; i++) {
                float a = start + (IM_PI - i * offset);
                ImColor c = ImColor(63, 140, 254, 255);
                c.Value.w = ImMax(0.0f, ImSin(a * heightSpeed));
                window->DrawList->AddCircleFilled(
                    ImVec2(pos.x + style.FramePadding.x +
                        i * (thickness * nextItemKoeff),
                        centre.y),
                    thickness, c, 90);
            }
        }
    ...
    void SpinnerFadeDots(const char* label, float thickness, const ImColor& color = 0xffffffff, float speed = 2.8f, size_t dots = 3)
    {
        ...
        const float nextItemKoeff = 2.8f;
        const float heightKoeff = 2.f;
        const float heightSpeed = 0.8f;
        ImVec2 pos = window->DC.CursorPos;
        ...
        for (size_t i = 0; i < dots; i++)
        {
            ...
            window->DrawList->AddCircleFilled(ImVec2(pos.x + style.FramePadding.x + i * (thickness * nextItemKoeff), centre.y), thickness, c, 25);
        }
    }
